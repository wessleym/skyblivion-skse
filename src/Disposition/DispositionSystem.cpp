...
bool DispositionSystem::Initialize() {
    TESNPCActivateHook::Apply();
    return SKSE::GetMessagingInterface()->RegisterListener(OnMessage);
}

void DispositionSystem::OnMessage(SKSE::MessagingInterface::Message* msg) {
    if (msg->type == SKSE::MessagingInterface::kDataLoaded) {
        REX::INFO("Looking up Factions, Globals, and Races...");
        LookUpFactions();
        LookUpGlobals();
        LookUpRaces();
        REX::INFO("Looking up Factions, Globals, and Races Complete");
    }
}
...
float DispositionSystem::CalcDisposition(RE::Actor* npc) {
    ...
    auto playerAVOwner = static_cast<RE::ActorValueOwner*>(player);
    auto npcAVOwner = static_cast<RE::ActorValueOwner*>(npc);
...
void DispositionSystem::PapyrusSetActorValue(RE::TESObjectREFR* a_ref, RE::BSFixedString valueName, float value) {
    auto actor = a_ref ? a_ref->As<RE::Actor>() : nullptr;
    if (!actor || valueName.empty()) {
        return;
    }

    using func_t = std::int64_t(*)(
        std::int64_t,
        std::uint32_t,
        RE::Actor*,
        const char**,
    ...
    static REL::Relocation<func_t> func{
        REL::ID(54743)  // SE ID = 54743, VR OFFSET = 0x9865D0
    };

    const char* avName = valueName.c_str();
    func(0, 0, actor, &avName, value);
}

void DispositionSystem::SetActorDisposition(RE::Actor* actor, std::string_view edid) {
    auto avOwner = static_cast<RE::ActorValueOwner*>(actor);
    if (!avOwner) {
        REX::INFO("No ActorValueOwner for {}", edid);
        return;
    }

    const auto currentDisp = avOwner->GetActorValue(kDispositionAV);

    REX::DEBUG("actor {} disp == {}", edid, currentDisp);
    ...
    if (currentDisp == 0.0f) {
        ...
        PapyrusSetActorValue(actor, kDispositionAVName, disposition);

        REX::DEBUG("set actor {} disp to {}", edid, disposition);
    }
}
