...
class GamepadDebugDisplay {
    ...
    public static install() {
        ...
        const onButtonEvent = (kind: "down" | "up", e: Event) => {
    ...
    private static makeLine(topPx: number) {
    ...
    private static addPollListener(callback: (message: string) => void) {
        ...
        let ticks = 0;
        const previouslyPressed: boolean[][] = [];
        setInterval(() => {
            ticks++;
            ...
            for (let p = 0; p < pads.length; p++) {
                ...
                padCount++;
                if (previouslyPressed[p] == null) { previouslyPressed[p] = []; }
...
function addEscapeListener(callback: () => void, preCallbackCheck: (() => boolean) | null = null) {
    document.addEventListener("keydown", (e) => {
        const escape = e.key == "Escape" || (<any>e).keyCode == 27;//PrismaUI doesn't seem to include .key. <any> suppresses the warning of keyCode.
